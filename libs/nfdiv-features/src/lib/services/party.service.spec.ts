...
describe("Http testing PartyService", () => {
  ...
  let partiesURL = `${BASE_PATH}${API_PATH}/party`;
  let getPartiesURL = `${BASE_PATH}${API_PATH}/party?caseId=1669996761194595`;
  let caseId = "1669996761194595";
  let testData = [
    {
  ...
  let addItem = { id: "", firstName: "Testy", lastName: "Geezer" };
  let data: Party[];
  ...
  beforeEach(() => {
    ...
    spectator.service.getParties(caseId);
    const request = spectator.expectOne(getPartiesURL, HttpMethod.GET);
  ...
  it("it should update an party using an ID and an attribute", () => {
    let data;
    spectator.service.getParties(caseId);
    const request0 = spectator.expectOne(getPartiesURL, HttpMethod.GET);
    request0.flush(testData);

    spectator.service
      .updateParty("12345", { firstName: "Freddy" })
      .subscribe((response) => {
        data = [response];
      });
  ...
  it("it should delete  a party using an ID and an attribute", () => {
    ...
    const request = spectator.expectOne(
      partiesURL + "/id/2d532789-32d5-49fa-a030-a72634b6196d",
      HttpMethod.DELETE
    );
    request.flush({
      id: "2d532789-32d5-49fa-a030-a72634b6196d",
